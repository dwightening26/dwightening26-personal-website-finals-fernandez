    ...
    <div className="page">
      ...
      {lightbox !== null && (
        <div className="lightbox" onClick={() => setLightbox(null)}>
          ...
          />
          {lightbox > 0 && (
            <button className="lightbox-nav lightbox-prev" onClick={e => { e.stopPropagation(); setLightbox(lightbox - 1); }}>‹</button>
          )}
          {lightbox < activePhotos.length - 1 && (
            <button className="lightbox-nav lightbox-next" onClick={e => { e.stopPropagation(); setLightbox(lightbox + 1); }}>›</button>
          )}
        </div>
      )}
            ...
            <div key={i} className="skills-group">
              ...
              <div className="skills-wrap">
                {group.items.map((skill) => (
                  <div key={skill.name} className="skill-icon" data-tooltip={skill.name}>{skill.icon}</div>
                ))}
              </div>
            </div>
          ))}
      ...
      <section ref={galleryRef} className="section reveal">
        ...
        <div className="gallery-tabs">
          {GALLERY.map(g => (
            <button
              key={g.category}
              className={`gallery-tab ${activeCategory === g.category ? 'active' : ''}`}
            ...
            {entries.map((entry, i) => (
              <div key={entry.id} className="entry-item" style={{ transitionDelay: `${i * 60}ms` }}>
                <div className="entry-top">
                  ...
                  <span className="entry-date">{new Date(entry.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                </div>
