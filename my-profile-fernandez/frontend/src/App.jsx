          ...
          <div className="entries-list">
            {entries.map((entry, i) => (
              <div key={entry.id} className="entry-item" style={{ transitionDelay: `${i * 60}ms` }}>
                ...
                <button className={`like-btn ${likedIds.includes(entry.id) ? 'liked' : ''}`} onClick={() => handleLike(entry.id, entry.likes || 0)} disabled={likedIds.includes(entry.id)}>♥ <span className="like-count">{entry.likes || 0}</span></button>
              </div>
            ))}
          </div>
        )}
